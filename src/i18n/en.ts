import type { TranslationKey } from './ky';

export const en: Record<TranslationKey, string> = {
  'common.save': 'Save',
  'common.cancel': 'No',
  'common.back': 'Back',
  'common.delete': 'Delete',
  'common.email': 'Email',
  'common.emailPlaceholder': 'you@example.com',
  'common.password': 'Password',
  'common.passwordPlaceholder': '••••••••',
  'common.fillAllFields': 'Please fill in all fields',

  'nav.appName': 'Өмүр',
  'nav.dashboard': 'Home',
  'nav.doctors': 'Doctors',
  'nav.appointment': 'Book an appointment',
  'nav.chat': 'Online consultation',
  'nav.labResults': 'Lab results',
  'nav.medications': 'Medications',
  'nav.vitals': 'Health vitals',
  'nav.medicalRecord': 'Medical record',
  'nav.sos': 'Emergency',
  'nav.reminders': 'Reminders',
  'nav.tips': 'Health tips',
  'nav.settings': 'Profile',

  'onboarding.skip': 'Skip',
  'onboarding.next': 'Next',
  'onboarding.start': 'Get started',
  'onboarding.step1.title': 'Track your health',
  'onboarding.step1.description': 'Save your blood pressure, pulse, and SpO2 readings and see their history.',
  'onboarding.step2.title': 'Add a measurement',
  'onboarding.step2.description': 'Tap "+" on the home screen and enter your reading manually.',
  'onboarding.step3.title': 'Reminders',
  'onboarding.step3.description': "Set a reminder so you never forget to take medicine or measure a reading.",
  'onboarding.step4.title': 'SOS button',
  'onboarding.step4.description': 'In an emergency, it sends your emergency contacts your last readings.',

  'login.title': 'Welcome back',
  'login.subtitle': 'Enter your details to sign in',
  'login.errorRequired': 'Enter your email and password',
  'login.submit': 'Sign in',
  'login.createAccount': 'Create account',
  'login.forgotPassword': 'Forgot password?',

  'signup.title': 'Create account',
  'signup.subtitle': 'Sign up to track your health',
  'signup.submit': 'Sign up',
  'signup.haveAccount': 'I already have an account',
  'signup.doctorToggle': "I'm a doctor (verified via license/diploma photo)",
  'signup.doctorDisclaimer': "Registering as a doctor requires uploading a photo of your license or diploma. Your account stays \"pending\" until the app's administrator reviews it manually. This isn't a full verification system — just a simple check available to a single administrator.",
  'signup.licenseImagePick': 'Choose license/diploma photo',
  'signup.licenseImageChange': 'Change photo',
  'signup.licenseImageRequired': 'Registering as a doctor requires uploading a photo of your license/diploma',
  'signup.licenseImagePermissionDenied': 'Gallery access is required to choose a photo',

  'auth.invalidEmail': 'Invalid email address',
  'auth.invalidPassword': 'Password must be at least 6 characters',
  'auth.emailTaken': 'An account with this email already exists',
  'auth.invalidCredentials': 'Incorrect email or password',
  'auth.networkError': "Couldn't reach the server. Check that it's running.",

  'forgotPassword.title': 'Reset your password',
  'forgotPassword.subtitle': "Enter the email you signed up with — we'll send you a reset code",
  'forgotPassword.submit': 'Send code',
  'forgotPassword.sent': 'If an account exists for this email, a password reset code has been sent (valid for 1 hour).',
  'forgotPassword.enterCode': 'Enter code',
  'forgotPassword.backToLogin': 'Back to sign in',

  'resetPassword.title': 'Set a new password',
  'resetPassword.subtitle': 'Enter the code from your email and a new password',
  'resetPassword.emailLabel': 'Email',
  'resetPassword.codeLabel': 'Code',
  'resetPassword.codePlaceholder': 'XXXXXXXX',
  'resetPassword.newPasswordLabel': 'New password',
  'resetPassword.confirmPasswordLabel': 'Confirm new password',
  'resetPassword.submit': 'Update password',
  'resetPassword.errorMismatch': "Passwords don't match",
  'resetPassword.invalidCode': 'The code is invalid or has expired',

  'profileSetup.titleCreate': 'Set up your profile',
  'profileSetup.titleEdit': 'Edit profile',
  'profileSetup.subtitle': 'Basic info is needed to calculate your readings accurately',
  'profileSetup.genderLabel': 'Gender',
  'profileSetup.ageLabel': 'Age',
  'profileSetup.agePlaceholder': '30',
  'profileSetup.heightLabel': 'Height (cm)',
  'profileSetup.heightPlaceholder': '165',
  'profileSetup.weightLabel': 'Weight (kg)',
  'profileSetup.weightPlaceholder': '60',
  'profileSetup.submitCreate': 'Continue',

  'gender.female': 'Female',
  'gender.male': 'Male',

  'dashboard.greeting': 'Hello!',
  'dashboard.subtitle': "Today's readings",
  'dashboard.streak': 'Last 7 days',

  'day.short.0': 'Su',
  'day.short.1': 'Mo',
  'day.short.2': 'Tu',
  'day.short.3': 'We',
  'day.short.4': 'Th',
  'day.short.5': 'Fr',
  'day.short.6': 'Sa',

  'quickLink.history': 'History',
  'quickLink.reminders': 'Reminders',
  'quickLink.settings': 'Settings',
  'quickLink.insights': 'Insights',
  'quickLink.sos': 'SOS',
  'quickLink.telemedicine': 'Doctor',

  'emergencyContacts.title': 'Emergency contacts',
  'emergencyContacts.empty': 'No contacts yet. Add your first one via the "+" above.',
  'emergencyContacts.addTitle': 'Add contact',
  'emergencyContacts.nameLabel': 'Name',
  'emergencyContacts.namePlaceholder': 'Jane Doe',
  'emergencyContacts.phoneLabel': 'Phone number',
  'emergencyContacts.phonePlaceholder': '+996 700 123 456',
  'emergencyContacts.errorNameRequired': 'Enter a name',
  'emergencyContacts.errorPhoneInvalid': 'Enter a valid phone number',

  'sos.title': 'SOS',
  'sos.subtitle': "In an emergency, an SMS or call will go out to your contacts",
  'sos.send': 'Send SOS',
  'sos.noContacts': 'Add at least one emergency contact first.',
  'sos.addContacts': 'Add contact',
  'sos.manageContacts': 'Manage contacts',
  'sos.message': 'SOS! I need help. My latest readings: {details}',
  'sos.noMeasurements': 'no measurements',
  'sos.smsUnavailable': "SMS isn't available on this device. A call to your first contact will open instead.",

  'chat.title': 'Chat with doctor',
  'chat.disclaimer': "Doctor registration in this app isn't verified (MVP). In a real emergency, call your local emergency number.",
  'chat.placeholder': 'Type a message...',
  'chat.send': 'Send',
  'chat.empty': 'No messages yet. Write your question to the doctor.',
  'chat.errorEmpty': 'Message cannot be empty',

  'doctorInbox.title': 'Patients',
  'doctorInbox.empty': "No one has written yet.",
  'doctorInbox.pending': "Your registration is being reviewed by an administrator. You'll be able to see patient messages once it's approved.",
  'doctorInbox.rejected': 'Your registration was rejected. Contact the app administrator for details.',

  'bluetooth.title': 'Bluetooth',
  'bluetooth.subtitle': 'Scan to find your blood pressure monitor or pulse oximeter',
  'bluetooth.unsupported': "Bluetooth isn't supported on web or in Expo Go — it needs a custom dev client build.",
  'bluetooth.startScan': 'Scan',
  'bluetooth.stopScan': 'Stop',
  'bluetooth.noDevices': 'No devices found',
  'bluetooth.waitingReading': 'Waiting for a reading...',
  'bluetooth.disconnect': 'Disconnect',
  'bluetooth.saved': 'Saved',

  'measurement.cameraOption': 'Measure with camera',
  'camera.title': 'Camera pulse measurement',
  'camera.instructions': 'Fully cover the back camera and flash with your finger, then tap "Start".',
  'camera.permissionRequired': 'Camera access needed',
  'camera.grantPermission': 'Grant permission',
  'camera.permissionDenied': 'Camera access was not granted. Allow it in your device settings and try again.',
  'camera.start': 'Start',
  'camera.measuring': 'Measuring...',
  'camera.holdStill': 'Hold your finger still, but don\'t press too hard',
  'camera.failed': 'Could not get a clear reading. Check that your finger fully covers the camera and flash, then try again.',
  'camera.retry': 'Try again',
  'camera.resultLabel': 'Estimated pulse',
  'camera.save': 'Save',
  'camera.disclaimer': 'This is not a medical-grade measurement — just a rough camera-based estimate. Use a certified blood pressure monitor/pulse oximeter for an accurate reading.',

  'steps.title': 'Steps',

  'sleep.title': 'Sleep',
  'sleep.disclaimer': "Experimental: if the phone stays still for over 3 hours, it's automatically counted as sleep, but background execution is limited by the OS and doesn't run on a fixed schedule — results may be inaccurate.",
  'sleep.checkNow': 'Check now',
  'sleep.checking': 'Measuring (2s)...',
  'sleep.lastSession': 'Last detected sleep',
  'sleep.duration': '{hours}h {minutes}m',
  'sleep.noSessions': 'No sleep detected yet.',
  'sleep.recentSessions': 'Recent entries',
  'sleep.currentlyStill': 'Right now: still',
  'sleep.currentlyMoving': 'Right now: moving',
  'sleep.unsupported': "Doesn't work on web — a device is needed.",

  'alert.title': 'Concerning reading!',
  'alert.body': '{label}: {value}. {advice}',
  'alert.channelName': 'Alerts',

  'insights.empty': 'Not enough data for analysis yet. Add a few measurements.',
  'insights.status.good': 'Normal',
  'insights.status.watch': 'Worth watching',
  'insights.status.concern': 'See a doctor',
  'insights.trend.up': 'Rising',
  'insights.trend.down': 'Falling',
  'insights.trend.stable': 'Stable',

  'measurement.add': 'Add measurement',
  'measurement.bloodPressure': 'Blood pressure',
  'measurement.pulse': 'Pulse',
  'measurement.spo2': 'SpO2',
  'measurement.typeLabel': 'Measurement type',
  'measurement.systolicLabel': 'Systolic',
  'measurement.systolicPlaceholder': '120',
  'measurement.diastolicLabel': 'Diastolic',
  'measurement.diastolicPlaceholder': '80',
  'measurement.pulseLabel': 'Pulse (bpm)',
  'measurement.pulsePlaceholder': '72',
  'measurement.spo2Label': 'SpO2 (%)',
  'measurement.spo2Placeholder': '98',

  'history.empty': 'No measurements yet. Add your first entry via "Add measurement".',
  'date.today': 'Today',
  'date.yesterday': 'Yesterday',
  'month.0': 'January',
  'month.1': 'February',
  'month.2': 'March',
  'month.3': 'April',
  'month.4': 'May',
  'month.5': 'June',
  'month.6': 'July',
  'month.7': 'August',
  'month.8': 'September',
  'month.9': 'October',
  'month.10': 'November',
  'month.11': 'December',

  'reminder.medicine': 'Take medicine',
  'reminder.measurement': 'Take measurement',
  'reminder.water': 'Drink water',
  'reminder.exercise': 'Exercise',
  'reminder.permissionUnsupportedList': 'Push notifications are not supported on web. Check on a mobile device.',
  'reminder.permissionDeniedList': 'Reminders need permission to send push notifications.',
  'reminder.grantPermission': 'Grant permission',
  'reminder.empty': 'No reminders yet. Add your first one via the "+" above.',
  'reminder.addTitle': 'New reminder',
  'reminder.permissionUnsupportedAdd': 'Push notifications are not supported on web. Check on a mobile device (Android/iOS).',
  'reminder.permissionDeniedAdd': 'Notification permission was not granted. The reminder will be saved, but no push notification will arrive.',
  'reminder.categoryLabel': 'Type',
  'reminder.titleLabel': 'Title',
  'reminder.titlePlaceholder': 'Take medicine',
  'reminder.timeLabel': 'Time (HH:MM)',
  'reminder.timePlaceholder': '08:00',
  'reminder.errorTitleRequired': 'Enter a title',
  'reminder.errorTimeInvalid': 'Enter time in HH:MM format (e.g. 08:00)',
  'reminder.channelName': 'Reminders',
  'reminder.taken': 'Done',
  'reminder.skip': 'Skip',
  'reminder.weeklyStats': 'This week: {taken}/{total}',

  'settings.profileSection': 'Profile',
  'settings.profileSummary': '{gender}, {age} years old',
  'settings.profileMetrics': '{height} cm · {weight} kg',
  'settings.profileEmpty': 'Profile not filled in',
  'settings.themeSection': 'Theme',
  'settings.accessibilitySection': 'Accessibility',
  'accessibility.largeText': 'Large text',
  'accessibility.largeTextDescription': 'Makes all text on screen bigger',
  'accessibility.highContrast': 'High contrast',
  'accessibility.highContrastDescription': 'Black-and-white colors, easier to read',
  'settings.languageSection': 'Language',
  'settings.dataSection': 'Data',
  'settings.clearConfirm': 'All measurements and reminders will be deleted. Are you sure?',
  'settings.clearConfirmYes': 'Yes, clear',
  'settings.clearAll': 'Clear all data',
  'settings.onboardingReplay': 'App introduction',
  'settings.legalSection': 'Legal',
  'settings.privacyPolicy': 'Privacy Policy',
  'settings.termsOfService': 'Terms of Service',
  'settings.logout': 'Log out',

  'theme.system': 'System',
  'theme.light': 'Light',
  'theme.dark': 'Dark',

  'disclaimer.text': 'This is not a medical diagnosis. Consult a doctor if you have concerns.',

  'doctors.title': 'Doctors',
  'doctors.empty': 'No verified doctors yet.',

  'appointment.title': 'Book an appointment',
  'appointment.nameLabel': 'Your name',
  'appointment.namePlaceholder': 'Aigul Osmonova',
  'appointment.reasonLabel': 'Reason',
  'appointment.reasonPlaceholder': 'Blood pressure consultation',
  'appointment.whenLabel': 'Preferred date/time',
  'appointment.whenPlaceholder': 'August 27, before noon',
  'appointment.errorRequired': 'Please fill in all fields',
  'appointment.messageTemplate': '📅 Appointment request\nName: {name}\nReason: {reason}\nPreferred time: {when}',
  'appointment.submit': 'Send',
  'appointment.success': 'Your request was sent. You can see the reply in "Online consultation".',

  'labResults.title': 'Lab results',
  'labResults.empty': 'No lab results yet. Add the first one via "+" above.',
  'labResults.addTitle': 'New lab result',
  'labResults.nameLabel': 'Test name',
  'labResults.namePlaceholder': 'Hemoglobin',
  'labResults.valueLabel': 'Value',
  'labResults.valuePlaceholder': '135 g/L',
  'labResults.errorNameRequired': 'Enter a name',
  'labResults.errorValueRequired': 'Enter a value',

  'medications.title': 'Medications',
  'medications.empty': 'No medications yet. Add the first one via "+" above.',
  'medications.addTitle': 'New medication',
  'medications.nameLabel': 'Medication name',
  'medications.namePlaceholder': 'Paracetamol',
  'medications.dosageLabel': 'Dosage',
  'medications.dosagePlaceholder': '1 tablet, twice a day',
  'medications.errorNameRequired': 'Enter a name',
  'medications.errorDosageRequired': 'Enter a dosage',

  'medicalRecord.title': 'Medical record',
  'medicalRecord.profileSection': 'Personal info',
  'medicalRecord.vitalsSection': 'Latest vitals',
  'medicalRecord.medicationsSection': 'Medications',
  'medicalRecord.labResultsSection': 'Lab results',
  'medicalRecord.emptySection': 'No data',

  'healthTips.title': 'Health tips',
  'healthTips.tip1Title': 'Monitor your blood pressure regularly',
  'healthTips.tip1Body': 'Measure your blood pressure at the same time every day and keep a history — this helps your doctor make an accurate diagnosis.',
  'healthTips.tip2Title': 'Eat well',
  'healthTips.tip2Body': 'Reduce salt and eat more vegetables and fruit. This improves blood pressure and heart health.',
  'healthTips.tip3Title': 'Get light exercise',
  'healthTips.tip3Body': 'Walking at least 20-30 minutes a day is good for your cardiovascular system.',
  'healthTips.tip4Title': 'Keep a sleep routine',
  'healthTips.tip4Body': 'Lack of sleep raises blood pressure and harms your health. Aim for 7-8 hours a night.',
  'healthTips.tip5Title': 'Take medication on time',
  'healthTips.tip5Body': 'Take doctor-prescribed medication on time, without forgetting — set reminders.',
};
