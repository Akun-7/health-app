...
## Чоң тамга/жогорку контраст режими ("Production readiness" #5, 1-бөлүк)
...
- ✅ 2026-08-03тө чыныгы браузерде (Chrome, `npx expo start --web`) кол менен текшерилди: эки toggle тең иштейт, `Dashboard`догу үч `VitalCard` жогорку контраст режиминде так чек аралары менен ажыратылып көрүнөт, "Жеткиликтүүлүк" бөлүмүнүн өзүндөгү текст да чоңоюп, layout бузулбайт.

## Биринчи ачылуудагы onboarding ("Production readiness" #5, 2-бөлүк)

2026-08-03тө ишке ашырылды. Максат — жаны колдонуучуга (өзгөчө улгайган) тиркеменин негизги мүмкүнчүлүктөрүн тез, аз текст менен түшүндүрүү.

- **`OnboardingContext`** (`src/context/OnboardingContext.tsx`) — `seen: boolean` талаасын `health-app/onboardingSeen`де сактайт (жөнөкөй `AsyncStorage`, шифрленбейт — медициналык дайын эмес). `ComposeProviders`га `App.tsx`та `ThemeProvider`дан кийин, `AuthProvider`дан мурда кошулду.
- **`OnboardingScreen`** (`src/screens/OnboardingScreen.tsx`, 108 сап) — 4 бет (`steps` массиви): (1) тиркеме эмне үчүн, (2) өлчөө кантип кошулат, (3) эскертмелер кантип иштейт, (4) SOS эмне. Ар бир бетте чоң иконка (`@tabler/icons-react-native`), `typography.h1` аталыш, `typography.body` кыска сүрөттөмө (эки-үч сүйлөм, "аз текст" талабы боюнча), pagination точколору. Баскычтар: "Өткөрүп жиберүү" (баардык беттерде акыркысынан башка) жана "Кийинки"/"Баштоо" (акыркы бетте). Экөө тең `finish()`ти чакырат — жалгыз айырма ортодогу беттерде `setIndex` менен кийинки бетке өтөт же түз бүтүрөт.
- **Багыттоо логикасы бөлүшүлдү:** `src/navigation/resolveHomeRoute.ts` — `user`/`profile`ге жараша `Login`/`DoctorInbox`/`ProfileSetup`/`Dashboard`ду тандаган функция, мурда `RootNavigator.tsx`деги inline ternary болчу, эми `RootNavigator` да, `OnboardingScreen.finish()` да ушул бир функцияны колдонот (эки жерде дал келбей калуу тобокелдигин жоготот).
- **`RootNavigator`деги `initialRouteName`:** эми биринчи `!onboardingSeen ? 'Onboarding' : resolveHomeRoute(user, profile)` — башка бардык `authLoading`/`profileLoading`/`onboardingLoading`оо `NavigationContainer` мурунтан эле кечиктирет (жаны `onboardingLoading` ошол эле гейтке кошулду).
- **Экинчи жолу көрүү:** `SettingsScreen`ге "Колдонмо менен таанышуу" (`SettingsLinkRow`) кошулду, профиль бөлүмүнөн кийин — `navigation.navigate('Onboarding')`. `OnboardingScreen.finish()` `navigation.canGoBack()`ду текшерет: эгер true болсо (Settings'тен ачылган учур) `goBack()`, эгер false болсо (биринчи ачылуу, стектин башы) `reset()` менен `resolveHomeRoute`го багытталат — ушундай жол менен эки сценарий тең бир кодду бөлүшөт, бирок Settings'тен ачылганда бүт стек кайра түзүлбөйт.
- i18n: `onboarding.*` (skip/next/start + 4 бет үчүн `stepN.title`/`stepN.description`) жана `settings.onboardingReplay` — ky/ru/en үчөө тең кошулду.
- ✅ 2026-08-03тө чыныгы браузерде толук текшерилди: биринчи ачылуу (localStorage'дон `onboardingSeen` тазаланып) 4 бетти тең туура көрсөттү, "Баштоо" басылганда профили/аккаунту бар колдонуучу түз `Dashboard`го багытталды (`resolveHomeRoute` иштээри ырасталды), `Settings`тен "Колдонмо менен таанышуу" аркылуу кайра ачылганда Skip баскычы `Dashboard`го reset кылбастан, так `Settings`ке `goBack()` менен кайтарды.

⚠️ **Дагы эле жок:** onboarding'дин өзү автоматтык тест менен капталган эмес (component/screen тесттери — `LoginScreen`/`SignUpScreen`деги сыяктуу — өзүнчө кошулушу мүмкүн, бирок бул тапшырмада суралган эмес). Түзмөктө (Android/iOS, чыныгы экран өлчөмдөрүндө) визуалдык текшерилген эмес — веб-браузерде гана.
...
health-app/
  ...
  eas.json           — EAS build профилдери: development/preview/production (+submit.production)
  docs/              — GitHub Pages'ке жарыяланган купуялык саясаты/шарттар (ky/ru/en, 6 файл + index.md)
  store-listing/     — Play Store жарыясы үчүн эталон материалдар (GitHub Pages'ке жарыяланбайт): listing-ky/ru/en.md + CHECKLIST.md
  src/
...
5. ✅ **Улгайган колдонуучуга ылайыктоо — 2026-08-03тө бүттү:** `Settings`ке чоң тамга/жогорку контраст toggle'дору кошулду (толук деталь төмөндөгү "Чоң тамга/жогорку контраст режими" бөлүмүндө), андан кийин ошол эле күнү биринчи ачылуудагы 4-беттик onboarding кошулду (толук деталь төмөндөгү "Биринчи ачылуудагы onboarding" бөлүмүндө). Экөө тең чыныгы браузерде (Chrome, `npx expo start --web`) кол менен текшерилди — onboarding'дин 4 бети тең туура көрүнөт, аягында профили/аккаунту бар колдонуучу Dashboard'го туура багытталат, Settings'тен кайра ачылганда Skip/Баштоо `goBack()` менен Settings'ке кайтарат (reset эмес), эки accessibility toggle'у тең иштейт (жогорку контраст режиминде VitalCard'дар чек ара менен ажыратылып көрүнөт).
6. 🔶 **Play Store'го чыгарууга даярдык — 2026-07-30да башталды, 2026-08-03тө уланды:** Купуялык саясаты жана пайдалануу шарттары `docs/` папкасында (`docs/privacy-policy.md`, `docs/terms-of-service.md` — юридикалык консультация эмес, расмий жарыялаардын алдында юрист карашы сунушталат), GitHub Pages аркылуу `master` бранчынан жарыяланат. Бул үчүн репозиторий **private'дан public'ке которулду** (2026-07-30). 2026-08-03тө **орусча/англисче котормолору кошулду** (`docs/privacy-policy-ru.md`/`-en.md`, `docs/terms-of-service-ru.md`/`-en.md` — Кыргызча файлдардын аты өзгөртүлгөн жок, себеби ал URL мурунтан эле жарыяланган болушу мүмкүн эле), `docs/index.md` үчөө тең тилге шилтеме берет. **URL'лер:**
   - ky: https://akun-7.github.io/health-app/privacy-policy.html · https://akun-7.github.io/health-app/terms-of-service.html
   - ru: https://akun-7.github.io/health-app/privacy-policy-ru.html · https://akun-7.github.io/health-app/terms-of-service-ru.html
   - en: https://akun-7.github.io/health-app/privacy-policy-en.html · https://akun-7.github.io/health-app/terms-of-service-en.html

   `SettingsScreen`деги "Купуялык саясаты"/"Пайдалануу шарттары" шилтемелери эми `Record<Locale, string>` картасы аркылуу колдонуучунун учурдагы UI тилине жараша туура баракты ачат (мурда үчөө тең бир гана кыргызча URL'ге көрсөтчү).

   Ошол эле күнү **`eas.json`ге `production` build профили кошулду** (`android.buildType: "app-bundle"`, `autoIncrement: true`) — мурда `development`/`preview` гана бар болчу, Play Store'го тапшыруу үчүн `.aab` формат милдеттүү. `submit.production` бөлүмү да кошулду (азырынча бош — `eas submit` колдонуу үчүн Google Cloud service account JSON'ун колдонуучу өзү кийин кошушу керек).

   **`store-listing/` жаны папка кошулду** (GitHub Pages'ке жарыяланбайт, тек эталон/справка катары): `listing-ky.md`/`-ru.md`/`-en.md` — Play Console'го копия-паста кылууга даяр кыска/толук сүрөттөмө, жана `CHECKLIST.md` — Play Console'до толтурула турган Content rating/Data safety суроолоруна жооп-жобо, эмне бүткөн/эмне колдонуучунун өзү жасашы керек экени толук тизмеленген.

   **Дагы эле жок (толугу менен колдонуучунун өзү жасашы керек — Google Play Console аккаунту, $25 төлөм, же Claude Code'дун Bash sandbox'унан мүмкүн эмес браузердик аракеттер талап кылат):** Play Console аккаунту ачуу, `eas build --platform android --profile production` иштетүү, feature graphic (1024x500) жана скриншоттор түзүү (азырынча такыр жок), Store listing/Content rating/Data safety формаларын Play Console'до кол менен толтуруу, "Internal testing"тен баштап "Production"го чейин жарыялоо. Толук деталь — `store-listing/CHECKLIST.md`.
